...
#  newdata.pred<-makeNewdata(predlf,"Pred")
  ...
  if(overall.composition.plot){
    ...
    plot.data$Fishery<-
      ...
      if(verbose){cat("L281 ;");cat("colnames(plot.data):\n",colnames(plot.data),"\n")}
      ...
      if(fit){
        if(verbose){cat("L287 ; ");cat("colnames(plot.data):\n",colnames(plot.data),"\n")}
