...
years <- trunc(seq(year1,length=nyr,by=1/tsteps))
...
else
{
  ...
  ##--- aggregate by year
  B <- aggregate(B,list(years),mean)
  #year <- B[,1]
  #B <- B[,-1]
  Bnof0<-Bnof <- aggregate(Bnof,list(years),mean)
#  require(magrittr)
  SBf0<-Bnof[((nyr/tsteps)-windows[1]):((nyr/tsteps)-windows[2]+1),]%>% colMeans()
#  cat("L43\n");browser()
...
par(mfrow=plot.layout, mar=c(2,4,1,2)+.1, lwd=.5, xpd=T, omi=c(0,0.2,0,0))
  ...
  for(i in 1:nplt){
    ...
    plot(years,B[,i+1]/SBf0[i+1],type='l',ylim=c(0,ymax),ann=F,axes=F,lwd=2,col=1)
